...
# ds_mde = gdal.Open('D:/DANI/2023/TEMA_TORMENTAS/DATOS/INEGI/CEM_V3_20170619_R15_NACIONAL_TIF/CEM_15m_ITRF08.tif')
# tiff = rasterio.open('D:/DANI/2023/TEMA_TORMENTAS/DATOS/INEGI/CEM_V3_20170619_R15_NACIONAL_TIF/CEM_15m_ITRF08.tif')
...
cmap_classes = LinearSegmentedColormap.from_list('classes',["darkred", "cyan", "blue", "darkblue"], N=4)
...
# df.columns
# df_x = df.copy()
# df_x = df_x[df_x['State_Name']=='Nuevo León']
# df_x = df_x[df_x['timestep']=='Daily']
# df_x = df_x[df_x['timestep']=='Yearly']
# plt.scatter(df_x['Z_Range'], df_x['kge'], s=0.1, c=df_x['pbias'], cmap='jet_r', vmin=-100, vmax=100)
# plt.ylim(-1,1)
...
for period in periods_dict:
        ...
        for col, error in enumerate(errors_list):
            ...
            if error == 'pbias':
                ...
                df_plot = ax[row,col].matshow(arr1, cmap=cmap_rbg,
                                        vmin=vmin_dict[error], vmax=vmax_dict[error]) #levels=pplt.arange(vmin_dict[error], vmax_dict[error], 4)) #2*bias_lims/15
                df_plot_out1 = ax[row,col].matshow(arr_out1, cmap=cmap_dr2r,
                                        vmin=-100, vmax=vmin_dict[error]) #levels=pplt.arange(-100, vmin_dict[error], 4))
                df_plot_out2 = ax[row,col].matshow(arr_out2, cmap=cmap_g2dg,
                                    vmin=vmax_dict[error], vmax=100) #levels=pplt.arange(vmax_dict[error], 100, 4))
                ...
                df_plot = ax[row,col].matshow(arr1, cmap=cmap_b2r,
                                        vmin=vmin_dict[error], vmax=vmax_dict[error]) #levels=pplt.arange(vmin_dict[error], vmax_dict[error], 0.1)) #np.round((avmax-avmin)/15,2)
                df_plot_out1 = ax[row,col].matshow(arr_out1, cmap=cmap_r2dr, #, #, vmin=0) #,
                                        vmin=vmax_dict[error], vmax=2.8) #levels=pplt.arange(vmax_dict[error], 2.8, 0.1))
                ...
                df_plot = ax[row,col].matshow(arr1, cmap=cmap_r2b,
                                        vmin=vmin_dict[error], vmax=vmax_dict[error]) #levels=pplt.arange(vmin_dict[error], vmax_dict[error], 0.1)) #np.round((avmax-avmin)/15,2)
                df_plot_out1 = ax[row,col].matshow(arr_out1, cmap=cmap_dr2r,
                                        vmin=-1.8, vmax=vmin_dict[error]) #levels=pplt.arange(-1.8, vmin_dict[error], 0.1))
                ...
                df_plot = ax[row,col].matshow(arr1, cmap=cmap_r2b,
                                        vmin=vmin_dict[error], vmax=vmax_dict[error]) #levels=pplt.arange(vmin_dict[error], vmax_dict[error], (vmax_dict[error]-vmin_dict[error])/16)) #np.round((avmax-avmin)/15,2)
                df_plot_out1 = ax[row,col].matshow(arr_out1, cmap=cmap_dr2r,
                                        vmin=-4, vmax=vmin_dict[error]) #levels=pplt.arange(-4, vmin_dict[error], 0.4))
            ...
            if row == 4:
                ...
                if error == 'pbias':
                    ticks_loc1 = np.linspace(-100, -50, 3, endpoint=True)
                    ticks_loc2 = np.linspace(50, 100, 3, endpoint=True)
                    ...
                    cbar_out1.ax.set_xticklabels(['<-100', '-75', '-50'])
                    cbar_out2.ax.set_xticklabels(['50', '75', '>100'])
                elif error == 'rsr':
                    ticks_loc1 = np.linspace(vmax_dict[error], 2.8, 3, endpoint=True)
                    cbar_out1 = fig.colorbar(df_plot_out1, ticks=ticks_loc1, loc='b', col=(col+1), shrink=0.8, pad=0.15)
                    cbar_out1.ax.set_xticklabels([f'{vmax_dict[error]}', '2.1', '>2.8'])
                elif error == 'kge':
                    ticks_loc1 = np.linspace(-1.8, vmin_dict[error], 3, endpoint=True)
                    cbar_out1 = fig.colorbar(df_plot_out1, ticks=ticks_loc1, loc='b', col=(col+1), shrink=0.8, pad=0.15)
                    cbar_out1.ax.set_xticklabels(['<-1.8', '-1.1', f'{vmin_dict[error]}'])
                elif error == 'nashsutcliffe':
    ...
    plt.figtext(0.04, 0.105, 'Standard', size='medium', weight='normal')
    plt.figtext(0.04, 0.065, 'Extreme', size='medium', weight='normal')
    plt.figtext(0.94, 0.105, 'Standard', size='medium', weight='normal')
    plt.figtext(0.94, 0.065, 'Underestimation', size='medium', weight='normal')
    plt.figtext(0.94, 0.03, 'Overestimation', size='medium', weight='normal')
        ...
        for timestep in timesteps:
            ...
            for i, ds in enumerate(ds_order):
                ...
                for j, filter_criteria in criteria.iterrows():
                    ...
                    # Save matrix results in csv
                    # for e, error in enumerate(errors_list):
                    #     # criteria_errors_array[e,:,:]
                    #     df_errors_region = pd.DataFrame(criteria_errors_array[e,:,:], columns=ds_order)
                    #     df_errors_region.to_csv(f'D:/DANI/2023/TEMA_TORMENTAS/DATOS/VARIOS/Tables/ERROR_COMPARISON/Regions/{criteria_name}/Error_{error}_by_{criteria_name}_{periods_dict[period]}_{timestep}.csv')
                    #     # plt.matshow(criteria_errors_array[e,:,:])
            ...
            if criteria_name == 'Climate':
                ...
                plt.figtext(0.03, 0.15, 'Standard', size='medium', weight='normal')
                plt.figtext(0.03, 0.10, 'Extreme', size='medium', weight='normal')
                plt.figtext(0.95, 0.15, 'Standard', size='medium', weight='normal')
                plt.figtext(0.95, 0.10, 'Underestimation', size='medium', weight='normal')
                plt.figtext(0.95, 0.05, 'Overestimation', size='medium', weight='normal')
            elif criteria_name == 'Basin':
                plt.figtext(0.01, 0.98, f'{periods_dict[period]}\n{timesteps_names[timestep]}', size='large', weight='bold')
                plt.figtext(0.03, 0.083, 'Standard', size='medium', weight='normal')
                plt.figtext(0.03, 0.052, 'Extreme', size='medium', weight='normal')
                plt.figtext(0.95, 0.083, 'Standard', size='medium', weight='normal')
                plt.figtext(0.95, 0.052, 'Underestimation', size='medium', weight='normal')
                plt.figtext(0.95, 0.021, 'Overestimation', size='medium', weight='normal')
            elif criteria_name == 'State':
                plt.figtext(0.01, 0.97, f'{periods_dict[period]}\n{timesteps_names[timestep]}', size='large', weight='bold')
                plt.figtext(0.03, 0.091, 'Standard', size='medium', weight='normal')
                plt.figtext(0.03, 0.059, 'Extreme', size='medium', weight='normal')
                plt.figtext(0.95, 0.091, 'Standard', size='medium', weight='normal')
                plt.figtext(0.95, 0.059, 'Underestimation', size='medium', weight='normal')
                plt.figtext(0.95, 0.024, 'Overestimation', size='medium', weight='normal')
...
for period in periods_dict:
    # period = 2
    print(periods_dict[period])
    # fig, ax = pplt.subplots(ncols=4, nrows=3, share=False, figwidth=11, figheight=13) #3 daily and monthly
    fig, ax = pplt.subplots(ncols=4, nrows=2, share=False, figwidth=11, figheight=9.5) #2 yearly
    # fig, ax = pplt.subplots(ncols=4, nrows=5, share=False, figwidth=11, figheight=21)
    
    # for row, timestep in enumerate(timesteps[:3]): #3 daily and monthly
    for row, timestep in enumerate(timesteps[3:]): #2 yearly
        # timestep = 'd'
        print(row, timesteps_names[timestep])
    ...
    # plt.figtext(0.01, 0.96, f'{periods_dict[period]}\n{timesteps_names[timestep]}', size='large', weight='bold')
    #3 daily and monthly
    # plt.figtext(0.03, 0.086, 'Standard', size='medium', weight='normal')
    # plt.figtext(0.03, 0.054, 'Extreme', size='medium', weight='normal')
    # plt.figtext(0.945, 0.086, 'Standard', size='medium', weight='normal')
    # plt.figtext(0.945, 0.054, 'Underestimation', size='medium', weight='normal')
    # plt.figtext(0.945, 0.023, 'Overestimation', size='medium', weight='normal')
    #2 yearly
    plt.figtext(0.03, 0.12, 'Standard', size='medium', weight='normal')
    plt.figtext(0.03, 0.078, 'Extreme', size='medium', weight='normal')
    plt.figtext(0.945, 0.12, 'Standard', size='medium', weight='normal')
    plt.figtext(0.945, 0.078, 'Underestimation', size='medium', weight='normal')
    plt.figtext(0.945, 0.034, 'Overestimation', size='medium', weight='normal')
    # plt.show()
...
################################################################################
# Error comparison by region in one plot
...
shpStMx.crs = "epsg:4326"
shpStMx.set_index('ID', inplace=True)
shpStMx.index.name = None
shpStMx = shpStMx.sort_index()
shpStMx = shpStMx[['NOMBRE', 'geometry']]

shpBasin = "D:/DANI/2023/TEMA_TORMENTAS/DATOS/GEE/SHP/Hydrologic_Regions.shp"
shpBasin = gpd.read_file(shpBasin, bbox=None, mask=None, rows=None)
shpBasin.crs = "epsg:4326"
shpBasin['CLAVE'] = shpBasin['CLAVE'].astype(int)
shpBasin.set_index('CLAVE', inplace=True)
shpBasin.index.name = None
shpBasin = shpBasin.sort_index()
shpBasin = shpBasin[['NAME', 'geometry']]
...
sClim.crs = "epsg:4326"
sClim.set_index('Region', inplace=True)
sClim.index.name = None
sClim = sClim.sort_index()
sClim = sClim[['Climate', 'geometry']]

criteria_full_name = {'Basin':'Hydrologic Region', 'Climate':'Climatic Region', 'State':'Administrative Division'}
criteria_names = ['Basin', 'Climate', 'State']
#### Select Filter Criteria
filter_criteria_order = [basins, climates, states]
shpCriteria = [shpBasin, sClim, shpStMx]

for ix in range(3):
# ix = 2
    criteria_name = criteria_names[ix]
    criteria = filter_criteria_order[ix]
    shpCrit = shpCriteria[ix]
    ...
    for period in periods_dict:
        ...
        for timestep in timesteps:
            ...
            fig, ax = pplt.subplots(ncols=4, nrows=7, share=False, figwidth=8, figheight=10, wspace=0.3, hspace=0.8)
            for col, error in enumerate(errors_list):
                print(error)
                # col, error = 0, 'kge'
                df_error = pd.read_csv(f'D:/DANI/2023/TEMA_TORMENTAS/DATOS/VARIOS/Tables/ERROR_COMPARISON/Regions/{criteria_name}/Error_{error}_by_{criteria_name}_{periods_dict[period]}_{timestep}.csv')
                df_error.set_index('Unnamed: 0', drop=True, inplace=True)
                df_error.index.name = None
                df_error.index = df_error.index + 1
                # df_shp = pd.concat([shpCrit, df_error], axis=1)
                
                for row, ds in enumerate(ds_order):
                    # row, ds = 0, 'Pchirps'
                    sMx.boundary.plot(ax=ax[row,col], alpha=1, lw=0.25, color='k', zorder=2)
                    shpCrit.boundary.plot(ax=ax[row,col], alpha=1, lw=0.25, color='k', zorder=2)
                    
                    if error == 'pbias':
                        df_err = df_error[ds].copy()
                        df_err[((df_err<vmin_dict[error])|(df_err>vmax_dict[error]))] = np.nan
                        df_err_out1 = df_error[ds].copy()
                        df_err_out1[df_err_out1>=vmin_dict[error]] = np.nan
                        df_err_out2 = df_error[ds].copy()
                        df_err_out2[df_err_out2<=vmax_dict[error]] = np.nan
                        
                        df_shp = pd.concat([shpCrit, df_err], axis=1)
                        df_shp_out1 = pd.concat([shpCrit, df_err_out1], axis=1)
                        df_shp_out2 = pd.concat([shpCrit, df_err_out2], axis=1)
                        
                        limits = mpl.colors.Normalize(vmin=vmin_dict[error], vmax=vmax_dict[error], clip=False)
                        limits1 = mpl.colors.Normalize(vmin=-100, vmax=vmin_dict[error], clip=False)
                        limits2 = mpl.colors.Normalize(vmin=vmax_dict[error], vmax=100, clip=False)
                        
                        df_shp.plot(ax=ax[row,col], alpha=1, lw=0.25, column=ds, cmap=cmaps[error], norm=limits, zorder=1)
                        df_shp_out1.plot(ax=ax[row,col], alpha=1, lw=0.25, column=ds, cmap=cmap_dr2r, norm=limits1, zorder=1)
                        df_shp_out2.plot(ax=ax[row,col], alpha=1, lw=0.25, column=ds, cmap=cmap_g2dg, norm=limits2, zorder=1)
                        
                    elif error == 'rsr':
                        df_err = df_error[ds].copy()
                        df_err[df_err>vmax_dict[error]] = np.nan
                        df_err_out1 = df_error[ds].copy()
                        df_err_out1[df_err_out1<=vmax_dict[error]] = np.nan
                        
                        df_shp = pd.concat([shpCrit, df_err], axis=1)
                        df_shp_out1 = pd.concat([shpCrit, df_err_out1], axis=1)
                        
                        limits = mpl.colors.Normalize(vmin=vmin_dict[error], vmax=vmax_dict[error], clip=False)
                        limits1 = mpl.colors.Normalize(vmin=vmax_dict[error], vmax=2.8, clip=False)
                        
                        df_shp.plot(ax=ax[row,col], alpha=1, lw=0.25, column=ds, cmap=cmaps[error], norm=limits, zorder=1)
                        df_shp_out1.plot(ax=ax[row,col], alpha=1, lw=0.25, column=ds, cmap=cmap_r2dr, norm=limits1, zorder=1)
                        
                    elif error == 'kge':
                        df_err = df_error[ds].copy()
                        df_err[df_err<vmin_dict[error]] = np.nan
                        df_err_out1 = df_error[ds].copy()
                        df_err_out1[df_err_out1>=vmin_dict[error]] = np.nan
                        ...
                        limits = mpl.colors.Normalize(vmin=vmin_dict[error], vmax=vmax_dict[error], clip=False)
                        limits1 = mpl.colors.Normalize(vmin=-1.8, vmax=vmin_dict[error], clip=False)
                        ...
                        limits = mpl.colors.Normalize(vmin=vmin_dict[error], vmax=vmax_dict[error], clip=False)
                        limits1 = mpl.colors.Normalize(vmin=-4, vmax=vmin_dict[error], clip=False)
                        
                        df_shp.plot(ax=ax[row,col], alpha=1, lw=0.25, column=ds, cmap=cmaps[error], norm=limits, zorder=1)
                        df_shp_out1.plot(ax=ax[row,col], alpha=1, lw=0.25, column=ds, cmap=cmap_dr2r, norm=limits1, zorder=1)
                        
                xcbar = plt.cm.ScalarMappable(norm=limits, cmap=cmaps[error])
                ticks_loc = np.linspace(vmin_dict[error], vmax_dict[error], 3, endpoint=True)
                cbar = fig.colorbar(xcbar, loc='b', ticks=ticks_loc, col=(col+1), shrink=0.8)
                if error == 'pbias':
                    cbar.ax.set_xticklabels([f'{vmin_dict[error]}', f'{vmin_dict[error]+(vmax_dict[error]-vmin_dict[error])/2:.1f}', f'{vmax_dict[error]}'])
                elif error == 'rsr':
                    cbar.ax.set_xticklabels([f'{vmin_dict[error]}', f'{vmin_dict[error]+(vmax_dict[error]-vmin_dict[error])/2:.1f}', f'{vmax_dict[error]}'])
                ...
                if error == 'pbias':
                    xcbar1 = plt.cm.ScalarMappable(norm=limits1, cmap=cmap_dr2r)
                    xcbar2 = plt.cm.ScalarMappable(norm=limits2, cmap=cmap_g2dg)
                    ticks_loc1 = np.linspace(-100, -50, 3, endpoint=True)
                    ticks_loc2 = np.linspace(50, 100, 3, endpoint=True)
                    cbar_out1 = fig.colorbar(xcbar1, ticks=ticks_loc1, loc='b', col=(col+1), shrink=0.8, pad=0.15)
                    cbar_out2 = fig.colorbar(xcbar2, ticks=ticks_loc2, loc='b', col=(col+1), shrink=0.8, pad=0.15)
                    cbar_out1.ax.set_xticklabels(['<-100', '-75', '-50'])
                    cbar_out2.ax.set_xticklabels(['50', '75', '>100'])
                elif error == 'rsr':
                    xcbar1 = plt.cm.ScalarMappable(norm=limits1, cmap=cmap_r2dr)
                    ticks_loc1 = np.linspace(vmax_dict[error], 2.8, 3, endpoint=True)
                    cbar_out1 = fig.colorbar(xcbar1, ticks=ticks_loc1, loc='b', col=(col+1), shrink=0.8, pad=0.15)
                    cbar_out1.ax.set_xticklabels([f'{vmax_dict[error]}', '2.1', '>2.8'])
                elif error == 'kge':
            ...
            plt.figtext(0, 0.97, f'{periods_dict[period]}\n{timesteps_names[timestep]}', size='large', weight='bold')
            plt.figtext(0, 0.115, 'Standard', size='medium', weight='normal')
            plt.figtext(0, 0.07, 'Extreme', size='medium', weight='normal')
            plt.figtext(0.93, 0.115, 'Standard', size='medium', weight='normal')
            plt.figtext(0.93, 0.07, 'Underestimation', size='medium', weight='normal')
            plt.figtext(0.93, 0.03, 'Overestimation', size='medium', weight='normal')
            fig.savefig(f'D:/DANI/2023/TEMA_TORMENTAS/DATOS/VARIOS/Figures/ERROR_COMPARISON/{criteria_name}s/Error_GeoComparison_{criteria_name}_{periods_dict[period]}_{timestep}.jpg', format='jpg', dpi=1000, bbox_inches='tight')
            plt.close()
...
performance_classes = ['Unsatisfactory', 'Satisfactory', 'Good', 'Very Good']
df = df_all_ts.copy()
df['nashsutcliffe_class_value'] = df['nashsutcliffe_class'].replace(['Unsatisfactory', 'Satisfactory', 'Good', 'Very Good'], [1, 2, 3, 4])
df['kge_class_value'] = df['kge_class'].replace(['Unsatisfactory', 'Satisfactory', 'Good', 'Very Good'], [1, 2, 3, 4])
df['rsr_class_value'] = df['rsr_class'].replace(['Unsatisfactory', 'Satisfactory', 'Good', 'Very Good'], [1, 2, 3, 4])
df['pbias_class_value'] = df['pbias_class'].replace(['Unsatisfactory', 'Satisfactory', 'Good', 'Very Good'], [1, 2, 3, 4])

df = df[df['timestep']=='Monthly']
...
# for timestep in timesteps:
    ...
    # fig, ax = plt.subplots(figsize=(8,8))
    # df.plot(x='nashsutcliffe', y='rsr', kind='scatter', ax=ax, marker='.', s=2)
    # for l in nse_lims:
    #     ax.axvspan(l, 1.0, facecolor='green', alpha=0.25, zorder=1)
    # for l in rsr_lims:
    #     ax.axhspan(l, 0, facecolor='green', alpha=0.25, zorder=1)
    # ax.set_xlim(-0.5,1)
    # ax.set_ylim(0,1.5)
    # df.plot(x='nashsutcliffe', y='kge', kind='scatter', ax=ax)
    # df.plot(x='nashsutcliffe', y='pbias', kind='scatter', ax=ax)
    ...
    ax.legend(*handles_labels, loc='lower left', title='Performance NSE', title_fontsize=10, prop = {'size':9},
               markerscale=0.5, labelspacing=0.2, handletextpad=0.1, frameon=True, borderpad=0.5, borderaxespad=0.5)
    ax.text(-0.47, 68, 'a)', bbox=dict(facecolor='white', alpha=0.8, edgecolor='white'), zorder=10)
    ...
    ax.set_xticks([-0.41,0,0.3,0.5,0.75,1])
    ax.set_yticks([-0.5,0,0.5,0.65,0.75,1])
    ax.legend(*handles_labels, loc='lower left', title='Performance PBIAS', title_fontsize=10, prop = {'size':9},
               markerscale=0.5, labelspacing=0.2, handletextpad=0.1, frameon=True, borderpad=0.5, borderaxespad=0.5)
    ax.text(-0.47, 0.93, 'c)', bbox=dict(facecolor='white', alpha=0.8, edgecolor='white'), zorder=10)
    
    ax = fig.add_subplot(2, 2, 2)
    sns.scatterplot(x='nashsutcliffe', y='pbias', hue='kge_class', data=df, ax=ax, palette=pallete_class,
                    s=0.75, linewidth=0, zorder=5)
    for l in pbias_lims:
        ax.axhspan(l, -l, facecolor='green', alpha=0.25, zorder=1)
    for l in nse_lims:
    ...
    ax.set_yticks([-75,-50,-25,-10,0,10,25,50,75])
    ax.legend(*handles_labels, loc='lower left', title='Performance KGE', title_fontsize=10, prop = {'size':9},
               markerscale=0.5, labelspacing=0.2, handletextpad=0.1, frameon=True, borderpad=0.5, borderaxespad=0.5)
    ax.text(-0.47, 68, 'b)', bbox=dict(facecolor='white', alpha=0.8, edgecolor='white'), zorder=10)
    
    nse_limit = -0.5
    kge_limit = -0.5
    pbias_limit = 75
    mask_limits = ((df['nashsutcliffe']>nse_limit) & (df['kge']>kge_limit) & (df['pbias']<pbias_limit) & (df['pbias']>-pbias_limit))
    
    ax = fig.add_subplot(2, 2, 4, projection='3d')
    g = ax.scatter3D(df['kge'][mask_limits], df['pbias'][mask_limits], df['nashsutcliffe'][mask_limits], s=0.3, linewidth=0, alpha=0.7,
                 c=df['rsr_class_value'][mask_limits], cmap=cmap_classes)
    ...
    ax.set_xticks([-0.41,0,0.3,0.5,0.75,1])
    ax.set_yticks([-75,-50,-25,-10,0,10,25,50,75])
    ax.set_zticks([-0.5,0,0.5,0.65,0.75,1])
    ax.set_xlabel('KGE', rotation=-40)
    ax.set_ylabel('PBIAS')
    ax.set_zlabel('NSE', rotation=85)
    ax.view_init(45, 135)
    
    # legend_handles = np.concatenate([[g.legend_elements()[0]], [performance_classes]], axis=0)
    ax.legend(*handles_labels, loc='upper right', title='Performance RSR', title_fontsize=10, prop = {'size':9},
               markerscale=0.5, labelspacing=0.2, handletextpad=0.1, frameon=True, borderpad=0.5, borderaxespad=0.5)
    ax.text(1.40, 62, 1.50, 'd)', bbox=dict(facecolor='white', alpha=0.8, edgecolor='white'), zorder=10)
    
    # cmap = ListedColormap(sns.color_palette("husl", 256).as_hex())
    
    plt.subplots_adjust(wspace=0.15, hspace=0.15)
    # plt.savefig('D:/DANI/2023/TEMA_TORMENTAS/DATOS/VARIOS/Figures/3_ERRORS/4ERRORS_All.jpg', format='jpg', dpi=1000, bbox_inches='tight', pad_inches=0.25)
    # plt.savefig(f'D:/DANI/2023/TEMA_TORMENTAS/DATOS/VARIOS/Figures/3_ERRORS/4ERRORS_All_{timestep}.jpg', format='jpg', dpi=1000, bbox_inches='tight', pad_inches=0.25)
    plt.savefig(f'D:/DANI/2023/TEMA_TORMENTAS/DATOS/VARIOS/Figures/3_ERRORS/4ERRORS_All_{ds}.jpg', format='jpg', dpi=1000, bbox_inches='tight', pad_inches=0.25)
    # plt.savefig(f'D:/DANI/2023/TEMA_TORMENTAS/DATOS/VARIOS/Figures/3_ERRORS/3ERRORS_{timestep}.jpg', format='jpg', dpi=1000, bbox_inches='tight')
    # plt.savefig(f'D:/DANI/2023/TEMA_TORMENTAS/DATOS/VARIOS/Figures/3_ERRORS/3ERRORS_{ds}_{timestep}.jpg', format='jpg', dpi=1000, bbox_inches='tight')
    plt.close()
